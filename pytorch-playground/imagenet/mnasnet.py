...
class BERModule(torch.nn.Module):
    def __init__(self, seed_tensor = None, custom_module = None):
        super(BERModule, self).__init__()
        self.seed_tensor = seed_tensor
        self.custom_module = custom_module
    def forward(self, x):
        return self.custom_module.ber_uniform(x,self.seed_tensor)
...
class MNASNet(torch.nn.Module):
    ...
    def __init__(self, alpha, num_classes=1000, dropout=0.2):
        ...
        layers = [
            ...
            #BERModule(seed_tensor= self.seed_tensor[self.rand_idx], custom_module= self.custom_module  ),

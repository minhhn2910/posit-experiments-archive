...
def eval_model(model, ds, n_sample=None, ngpu=1, is_imagenet=False):
    ...
    n_passed = 0
    print (model.seed_tensor.dtype)
    #print (model.seed_tensor)

    if (hasattr(model, 'seed_tensor')):
        print ('inject BER to weights ')
        model_raw = model
    ...
    for idx, (data, target) in enumerate(tqdm.tqdm(ds, total=n_sample)):
        ...
        #if (iteration_count%10 == 0 and is_imagenet):
        #    model.module.model.seed_tensor = refresh_tensor(model.module.model.ones, model.module.model.total_elements)
        #    model.module.model.rand_idx = torch.randperm(model.module.model.seed_tensor.nelement())
        #    model.module.model.seed_tensor = model.module.model.seed_tensor[model.module.model.rand_idx]
    ...
    #print (type(correct1))
    isint = isinstance(correct1, int)
    if (isint):
        acc1 = correct1 * 1.0 / n_passed
        acc5 = correct5* 1.0 / n_passed
    else:
        acc1 = correct1.data.numpy() * 1.0 / n_passed
        acc5 = correct5.data.numpy() * 1.0 / n_passed
    #print (correct1.data.cpu().numpy())
